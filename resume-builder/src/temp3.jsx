...
const initialUserData = {
  ...
  imageUrl: "https://via.placeholder.com/180/E0E7FF/4338CA?text=Your+Photo" // Keep a placeholder for initial load
};
...
const UserResumePreview = forwardRef(({ data, font, themeColor, zoomLevel }, ref) => {
  ...
  const Section = ({ title, children, icon, className = "" }) => (
    <section className={`mb-8 ${className}`}>
      {title && (
        <h2 className={`text-2xl md:text-3xl font-bold ${themeColorClass} border-b-2 ${themeBorderClass} pb-3 mb-5 tracking-wide uppercase flex items-center`}>
          {icon && React.createElement(icon, { className: `mr-4 h-6 w-6 ${themeAccentClass}` })}
          {title}
        </h2>
      )}
      {children}
  ...
  return (
    <div
      ...
      className={`text-base text-gray-800 max-w-3xl mx-auto border bg-white rounded-xl shadow-2xl printable-area p-10 md:p-12`}
      style={{ fontFamily: font, transform: `scale(${zoomLevel})`, transformOrigin: 'top left' }}
    >
      {/* Header Section: Name, Title, Contact, and Image */}
      <header className="flex flex-col md:flex-row justify-between items-center pb-8 mb-8 border-b border-gray-200">
        <div className="text-center md:text-left flex-grow md:pr-8">
          <h1 className={`text-5xl md:text-6xl font-bold text-gray-900 tracking-tight leading-tight mb-2`}>{data.name}</h1>
          {data.jobTitle && <p className={`text-2xl md:text-3xl ${themeAccentClass} font-medium mb-5`}>{data.jobTitle}</p>}
          <div className="text-md text-gray-600 flex flex-wrap justify-center md:justify-start gap-x-6 gap-y-2">
            {data.address && <p className="flex items-center"><MapPin size={16} className="mr-2.5 shrink-0 text-gray-500" /> {data.address}</p>}
        ...
        {data.imageUrl && (
          <div className="mt-8 md:mt-0 md:ml-8 flex-shrink-0">
            <img
              src={data.imageUrl}
              alt="Profile"
              className={`w-40 h-40 md:w-48 md:h-48 rounded-full object-cover shadow-lg border-4 ${themeImageBorderClass}`}
            />
          </div>
        )}
      </header>
      ...
      {data.experience && data.experience.length > 0 && (
        <Section title="Experience" icon={Briefcase}>
          {data.experience.map((exp) => (
            <div key={exp.id} className="mb-6 last:mb-0 flex justify-between items-start">
              <div className="flex-1 pr-6">
                <h3 className="font-semibold text-gray-800 text-xl">{exp.title}</h3>
                <p className={`text-lg ${themeAccentClass} font-medium`}>{exp.company}</p>
                <p className="text-base italic text-gray-500 mb-2">{exp.location}</p>
                <ul className="list-inside text-md text-gray-700 space-y-1.5 mt-3">
                  {exp.points && exp.points.map((point, j) => (
                    point.trim() !== '' && <li key={j} className="break-inside-avoid-column flex items-center">
                      <span className={`mr-2.5 ${themeBulletClass}`}>&bull;</span> {point}
                    </li>
                  ))}
                </ul>
              </div>
              <div className="text-md text-gray-500 font-semibold text-right flex-shrink-0">
                {exp.duration}
              </div>
            </div>
          ))}
      ...
      {data.education && data.education.length > 0 && (
        <Section title="Education" icon={GraduationCap}>
          {data.education.map((edu) => (
            <div key={edu.id} className="mb-5 last:mb-0 flex justify-between items-start">
              <div className="flex-1 pr-6">
                <h3 className="text-xl font-medium text-gray-800">{edu.degree}</h3>
                <p className={`text-md ${themeAccentClass}`}>{edu.institution}</p>
              </div>
              <div className="text-sm text-gray-500 font-semibold text-right flex-shrink-0">
                {edu.year}
              </div>
            </div>
          ))}
      ...
      {data.skills && data.skills.length > 0 && (
        <Section title="Skills" icon={Lightbulb}>
          <ul className="columns-1 md:columns-2 list-inside text-md text-gray-700 space-y-1.5">
      ...
      {data.hobbies && data.hobbies.length > 0 && (
        <Section title="Hobbies" icon={Palette}>
          <ul className="text-md list-disc list-inside ml-5 space-y-1.5 pt-1">
  ...
  return (
    <div className="bg-white p-8 rounded-xl shadow-2xl space-y-8 max-w-2xl mx-auto">
      ...
      <FormSectionWrapper title="Personal Details">
        ...
        {/* MODIFIED: Replaced text input with file input for image */}
        <div>
                ...
                <div className="absolute left-0 right-0 mt-1.5 z-20">
                  <div className="mx-2 p-2 bg-white rounded-md shadow-2xl border border-gray-200">
                    ...
                    <button onClick={handleLoadJsonClick}
                      ...
                      <Sparkles size={18} className="mr-2.5 text-gray-500" /> AI Edit (Load JSON)
                    </button>
                    <input
                      type="file"
                      ref={fileInputRef}
